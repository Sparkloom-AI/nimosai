...
const ResponsiveImage = React.forwardRef<HTMLImageElement, ResponsiveImageProps>(({
  ...
  const imageClasses = cn("object-cover transition-transform duration-300", {
  ...
  const overlayClasses = cn("absolute inset-0", "bg-gradient-to-br from-primary/30 via-primary/10 to-accent/20");
  const overlayBottomClasses = cn("absolute inset-0", "bg-gradient-to-t from-background/80 via-transparent to-transparent");
  if (variant === "avatar" || variant === "avatar-sm" || variant === "avatar-lg") {
    return <div className={containerClasses}>
          <img ref={ref} alt={alt} className={imageClasses} {...props} />
        </div>;
  }
  return <div className={containerClasses}>
        <img ref={ref} alt={alt} className={imageClasses} {...props} />
