  ...
  return <div className="min-h-screen bg-background flex">
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          ...
          {step === 'email' && <div className="space-y-6">
              <form onSubmit={handleEmailSubmit} className="space-y-4">
                <div className="relative">
                  ...
                  <Input type="email" placeholder="Enter your email" value={email} onChange={e => setEmail(e.target.value)} className="pl-10 h-12" required disabled={emailCheckLoading} />
                </div>
                <Button type="submit" className="w-full h-12" disabled={emailCheckLoading}>
                  {emailCheckLoading ? <>
              ...
              <Button type="button" variant="outline" onClick={handleGoogleSignIn} disabled={isLoading || emailCheckLoading} className="w-full h-12">
                {isLoading ? <Loader2 className="w-4 h-4 mr-2 animate-spin" /> : <svg className="w-4 h-4 mr-2" viewBox="0 0 24 24">
          ...
          {/* Register Step */}
          {step === 'register' && <div className="space-y-6">
              {/* Location Detection Banner */}
              <LocationDetectionBanner detectedCountry={locationData.country} isDetected={detectedLocation.isDetected} isLoading={detectedLocation.isLoading} onEditLocation={handleEditLocation} />

              {/* Expandable Location Settings */}
              {showLocationSettings && <ExpandableLocationSettings locationData={locationData} onLocationDataChange={setLocationData} />}
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="firstName" type="text" placeholder="Enter your first name" value={firstName} onChange={e => setFirstName(e.target.value)} className="h-12" required />
                  </div>
                  <div>
                    <label htmlFor="lastName" className="block text-sm font-medium mb-2">
                      Last name
                    </label>
                    <Input id="lastName" type="text" placeholder="Enter your last name" value={lastName} onChange={e => setLastName(e.target.value)} className="h-12" required />
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="Enter a password" value={password} onChange={e => setPassword(e.target.value)} className="pr-10 h-12" required minLength={6} />
                    <Button type="button" variant="ghost" size="sm" className="absolute right-0 top-0 h-12 px-3" onClick={() => setShowPassword(!showPassword)}>
                      {showPassword ? <EyeOff className="h-4 w-4 text-muted-foreground" /> : <Eye className="h-4 w-4 text-muted-foreground" />}
                    </Button>
                  </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <MobilePrefixSelector value={locationData.phonePrefix} onValueChange={prefix => setLocationData(prev => ({
                  ...prev,
                  phonePrefix: prefix
                }))} />
                    <Input id="mobile" type="tel" placeholder="Enter your mobile number" value={phoneNumber} onChange={e => setPhoneNumber(e.target.value)} className="flex-1 h-12" />
                  </div>
                  {phoneNumber && <p className="text-xs text-muted-foreground mt-1">
                ...
                <Button type="submit" disabled={isLoading || !agreedToTerms} className="w-full h-12 bg-foreground text-background hover:bg-foreground/90">
                  {isLoading ? <>
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                      Creating account...
                    </> : 'Create account'}
      ...
      <div className="hidden lg:block flex-1 bg-muted relative overflow-hidden">
        <img src="/lovable-uploads/5101447c-92ce-49c1-8837-5de26eeff4b6.png" alt="Professional using Nimos" className="absolute inset-0 w-full h-full object-cover" />
