-- Drop existing functions and recreate them with new signatures

DROP FUNCTION IF EXISTS public.get_user_studios();
DROP FUNCTION IF EXISTS public.get_studio_by_id(uuid);
DROP FUNCTION IF EXISTS public.create_studio_with_data(text, text, text, text, text, text, text);
DROP FUNCTION IF EXISTS public.update_studio_data(uuid, text, text, text, text, text, text, text);
