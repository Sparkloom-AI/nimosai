-- Fix the ambiguous column reference in create_studio_with_data function
-- The issue is that business_category_id needs to be qualified with table alias in the final SELECT
...
BEGIN
  ...
  -- Return the created studio (FIX: properly qualify business_category_id with table alias)
