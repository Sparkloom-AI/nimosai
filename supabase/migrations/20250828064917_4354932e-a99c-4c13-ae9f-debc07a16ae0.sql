...
-- Recreate the function with updated return type (without business_category_id)
...
-- Drop and recreate other functions to remove business_category_id references
...
DROP FUNCTION IF EXISTS public.update_studio_data(uuid,text,text,uuid,text,text,text,text);
