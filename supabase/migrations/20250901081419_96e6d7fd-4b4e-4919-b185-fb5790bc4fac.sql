-- Update the create_studio_with_data function to also create a team member record
...
-- Backfill team member records for existing studio owners who don't have them
INSERT INTO public.team_members (
  studio_id,
  first_name,
  last_name,
...
SELECT 
  ur.studio_id,
  COALESCE(split_part(p.full_name, ' ', 1), 'Studio') as first_name,
  COALESCE(split_part(p.full_name, ' ', 2), 'Owner') as last_name,
  p.email,
  'Studio Owner' as job_title,
  'high'::permission_level,
  'full_time'::employment_type,
  true as is_bookable,
  '#2ECC71' as calendar_color, -- Emerald green from brand colors
  CURRENT_DATE as start_date
FROM public.user_roles ur
JOIN public.profiles p ON p.id = ur.user_id
WHERE ur.role = 'studio_owner'
  AND ur.studio_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.team_members tm 
    WHERE tm.studio_id = ur.studio_id 
