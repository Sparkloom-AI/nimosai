-- Update the database function to handle tiktok_url instead of twitter_url and linkedin_url
CREATE OR REPLACE FUNCTION public.update_studio_data(
  ...
  studio_country text DEFAULT NULL::text,
  studio_currency text DEFAULT NULL::text,
  studio_tax_included boolean DEFAULT NULL::boolean,
  studio_default_team_language text DEFAULT NULL::text,
  studio_default_client_language text DEFAULT NULL::text,
  studio_facebook_url text DEFAULT NULL::text,
  studio_instagram_url text DEFAULT NULL::text,
  studio_tiktok_url text DEFAULT NULL::text
)
RETURNS TABLE(id uuid, name text, description text, phone text, email text, website text, timezone text, country text, currency text, tax_included boolean, default_team_language text, default_client_language text, facebook_url text, instagram_url text, tiktok_url text, created_at timestamp with time zone, updated_at timestamp with time zone)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET 
    ...
    tiktok_url = COALESCE(studio_tiktok_url, studios.tiktok_url),
    updated_at = now()
  WHERE studios.id = studio_id;

  -- Return the updated studio
  SELECT s.id, s.name, s.description, s.phone, s.email, s.website, s.timezone, s.country, s.currency, s.tax_included, s.default_team_language, s.default_client_language, s.facebook_url, s.instagram_url, s.tiktok_url, s.created_at, s.updated_at
  INTO studio_record
  FROM public.studios s
  WHERE s.id = studio_id;

  RETURN QUERY SELECT studio_record.id, studio_record.name, studio_record.description, studio_record.phone, studio_record.email, studio_record.website, studio_record.timezone, studio_record.country, studio_record.currency, studio_record.tax_included, studio_record.default_team_language, studio_record.default_client_language, studio_record.facebook_url, studio_record.instagram_url, studio_record.tiktok_url, studio_record.created_at, studio_record.updated_at;
END;
$function$;